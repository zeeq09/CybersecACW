...
def compare_media(file1, file2):
    ...
    Handles image, audio, and video file formats.
    """
    if file1.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')):
        return compare_images(file1, file2)
    elif file1.lower().endswith(('.wav', '.mp3')):
        return compare_audio(file1, file2)
    elif file1.lower().endswith(('.mp4', '.avi', '.mov')):
        return compare_videos(file1, file2)
    else:
        messagebox.showerror("Error", "Unsupported media type for comparison.")
        return False


def compare_images(image1_path, image2_path):
    """
    Compare two images pixel by pixel and visually highlight the differences.
    A new image showing the differences will be generated.
    ...
    if img1.size != img2.size:
        messagebox.showerror("Image Comparison Result", "Images have different sizes.")
        return False
    ...
    # Create an image to display the differences
    diff_image = np.zeros_like(img1_np)

    # Compute pixel-level differences and create a visual diff image
    difference = np.abs(img1_np - img2_np)
    diff_pixels = np.any(difference > 0, axis=-1)  # True if any channel (R, G, B) differs

    # Highlight the differing pixels in red on the diff_image
    diff_image[diff_pixels] = [255, 0, 0]  # Red color for differing pixels

    # Calculate the percentage of different pixels
    num_differences = np.sum(diff_pixels)
    ...
    diff_output_path = image1_path.split('.')[0] + '_diff.png'
    diff_image_pil.save(diff_output_path)

    if num_differences == 0:
        messagebox.showinfo("Comparison Result", "The images are identical.")
        return True
    else:
        messagebox.showinfo("Comparison Result", f"The images differ by {diff_percentage:.2f}% of pixels.\n"
                                                 f"Difference image saved as {diff_output_path}")
...
def compare_audio(audio1_path, audio2_path):
    ...
    Returns True if the audio files are identical, False otherwise.
    """
    song1 = wave.open(audio1_path, mode='rb')
    song2 = wave.open(audio2_path, mode='rb')

    # Ensure audio files have the same parameters
    if song1.getparams() != song2.getparams():
        messagebox.showerror("Audio Comparison Result", "Audio files have different formats or channels.")
        return False

    # Read audio frames
    frames1 = song1.readframes(song1.getnframes())
    frames2 = song2.readframes(song2.getnframes())
    ...
    # Compare waveforms
    if frames1 == frames2:
        messagebox.showinfo("Comparison Result", "The audio files are identical.")
        return True
    else:
        # Calculate the difference in waveform
        diff = np.sum(np.abs(np.frombuffer(frames1, dtype='int16') - np.frombuffer(frames2, dtype='int16')))
...
def compare_videos(video1_path, video2_path):
    ...
    Returns True if the video files are identical, False otherwise.
    """
    ...
    # Compare each frame
    with tqdm(total=total_frames, desc="Comparing Videos", unit="frame") as pbar:
        while cap1.isOpened() and cap2.isOpened():
            ...
            # Compare the frames pixel by pixel
    ...
    if differences == 0:
        messagebox.showinfo("Comparison Result", "The video files are identical.")
        return True
    else:
        diff_percentage = (differences / frame_count) * 100
        messagebox.showinfo("Comparison Result", f"The videos differ in {diff_percentage:.2f}% of frames.")
        return False
...
class SteganographyApp(TkinterDnD.Tk):
    def __init__(self):
        ...
        # Compare media button
        self.compare_button = Button(button_frame, text="Compare Media", command=self.threaded_compare_media,
                                     font=("Verdana", 12), bg='#008080', fg='white', relief="raised")
        self.compare_button.grid(row=0, column=2, padx=10)
        self.compare_button.bind("<Enter>", lambda event: self.compare_button.config(bg='#007070'))  # On hover
        self.compare_button.bind("<Leave>", lambda event: self.compare_button.config(bg='#008080'))  # On leave
    ...
    def threaded_compare_media(self):
        threading.Thread(target=self.compare_media_action).start()

    def compare_media_action(self):
        # Compare the original media file with the encoded one (if it exists)
        original_media = self.media_file
        encoded_media = original_media.split('.')[0] + '_output.' + original_media.split('.')[-1]

        if os.path.exists(encoded_media):
            compare_media(original_media, encoded_media)
        else:
            messagebox.showerror("Error", "No encoded media file found for comparison.")
